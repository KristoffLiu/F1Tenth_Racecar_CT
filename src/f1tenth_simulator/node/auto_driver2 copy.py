...
def perception(data):
    ...
    if swerve_percep[0] or swf_percep[0] or lnf_percep[0] or bilateral_percep[0]:
        ...
        if swf_percep[0]:
            isfullspeed = False
            shift_gear(4) if simulator_mode == True else shift_gear(2)
        if swerve_percep[0]:
            isfullspeed = False
            shift_gear(4) if simulator_mode == True else shift_gear(2)
        if isfullspeed == True:
...
def swerve_perception():
    global distance_set
    if distance_set[90] < 2.0:
        if distance_set[90] < 0.9:
...
def short_wide_front_perception():
    ...
    for i in range(30):
        if distance_set[89 - i] < distance_set[91 + i]:
            if distance_set[90 - i] < 0.4:
                steering_angle = -0.4
                break
            if distance_set[89 - i] < 0.6:
                #set_speedmode(2)
                steering_angle = -0.20
                break
        else:
            if distance_set[91 + i] < 0.4:
                steering_angle = 0.4
                break
            if distance_set[91 + i] < 0.6:
                #set_speedmode(2)
                steering_angle = 0.20
                break
    else:
