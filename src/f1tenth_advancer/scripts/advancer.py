...
class F1tenthAdvancer:
        ...
        if isObstacle:
            try:
                ...
                if _maxleftangle > _maxrightangle:
                    input = _maxleftangle / 30
                    steering_delta += (easingfunctions.easeOutCubic(input)) * self.MAX_STEERING_ANGLE * 2
                else:
                    input = _maxrightangle / 30
                    steering_delta -= (easingfunctions.easeOutCubic(input)) * self.MAX_STEERING_ANGLE * 2
    ...
    def normal_steering(self, midpt_angle):
        steering_delta = 0
        if self.min_front_dist <= self.MIN_ROAD_WIDTH * 3:
            if midpt_angle > 0:
                input = midpt_angle / 75
                steering_delta += - easingfunctions.easeInSine(input) * self.MAX_STEERING_ANGLE * 0.6
            else:
                input = midpt_angle / 75
                steering_delta += easingfunctions.easeInSine(input)   * self.MAX_STEERING_ANGLE * 0.6
        else:
    ...
    def obstacle_detection(self):
        ...
        for i in range(50):
            right_end_dist = self.lidar_data[left_end_angle]
        ...
        if self.lidar_data[left_end_angle] > 2.5 or self.lidar_data[right_end_angle] > 2.5:
            return False, left_end_angle, right_end_angle, self.lidar_data[left_end_angle], self.lidar_data[right_end_angle]
...
if __name__ == '__main__':
    try:
        ...
        f1tenth = F1tenthAdvancer(True, 2.75, 3.5, 6.0)
